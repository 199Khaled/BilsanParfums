-- Stored Procedures for Table: DuftÖle

Use [BilsanDb];
Go


CREATE OR ALTER PROCEDURE SP_Get_DuftÖle_ByID
(
    @ID int
)
AS
...
CREATE OR ALTER PROCEDURE SP_Get_All_DuftÖle
AS
BEGIN
    BEGIN TRY
...
CREATE OR ALTER PROCEDURE SP_Add_DuftÖle
(
    @AlteNummer int = NULL,
    @ParfümCode nchar(10) = NULL,
    @Ölmenge int = NULL,
    @Öltype nvarchar(50) = NULL,
    @Aktivierungsdatum date = NULL,
    @NewID INT OUTPUT
...
BEGIN
    BEGIN TRY
        ...
        INSERT INTO DuftÖle ([AlteNummer],[ParfümCode],[Ölmenge],[Öltype],[Aktivierungsdatum])
        VALUES (    @AlteNummer,
    LTRIM(RTRIM(@ParfümCode)),
    @Ölmenge,
    LTRIM(RTRIM(@Öltype)),
    @Aktivierungsdatum
);
...
CREATE OR ALTER PROCEDURE SP_Update_DuftÖle_ByID
(
    @ID int,
    @AlteNummer int = NULL,
    @ParfümCode nchar(10) = NULL,
...
BEGIN
    BEGIN TRY
        ...
        IF F @ParfümCode IS NULL OR LTRIM(RTRIM(@ParfümCode)) = ''
        ...
        UPDATE DuftÖle
        SET     [AlteNummer] = @AlteNummer,
    [ParfümCode] = LTRIM(RTRIM(@ParfümCode)),
    [Ölmenge] = @Ölmenge,
    [Öltype] = LTRIM(RTRIM(@Öltype)),
    [Aktivierungsdatum] = @Aktivierungsdatum
...
CREATE OR ALTER PROCEDURE SP_Delete_DuftÖle_ByID
(
    @ID int
)
AS
BEGIN
    ...
    BEGIN TRY
        ...
        IF NOT EXISTS (SELECT 1 FROM DuftÖle WHERE ID = @ID)
        BEGIN
            EXEC SP_HandleError;
            RETURN;
        END
        ...
        DELETE FROM DuftÖle WHERE ID = @ID;
...
CREATE OR ALTER PROCEDURE SP_Search_DuftÖle_ByColumn
(
...
BEGIN
    BEGIN TRY
        ...
        SET @ActualColumn = 
            CASE @ColumnName
                WHEN 'ID' THEN 'ID'
        WHEN 'AlteNummer' THEN 'AlteNummer'
        WHEN 'ParfümCode' THEN 'ParfümCode'
        WHEN 'Ölmenge' THEN 'Ölmenge'
        WHEN 'Öltype' THEN 'Öltype'
        WHEN 'Aktivierungsdatum' THEN 'Aktivierungsdatum'
                ELSE NULL
            END;
        ...
        SET @SQL = N'SELECT * FROM ' + QUOTENAME('DuftÖle') + 
